...
def to_parquet(df, loc, **kwargs):
    ...
    resp = {'type': 's3-blob', 'data': {'uri': loc}}
...
def main(inputs, output_loc, out_manifest_loc):
    ...
    d0 = to_parquet(mkdata(rng, (num_train_rows, num_ids)),
                    f'{output_loc}/train.snappy.parquet', index=False)
    d1 = to_parquet(mkdata(rng, (num_test_rows, num_ids)),
                    f'{output_loc}/test.snappy.parquet', index=False)
    write_s3_json({'type': 'map', 'data': {'train': d0, 'test': d1}},
                  out_manifest_loc)
    return
