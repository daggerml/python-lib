...
run_hello_world() {
  ...
  dag_id=$(dml show | jq '.commit.dags["examples/00-hello-world"]' -r)
  node_id=$(dml dag get ${dag_id} | jq .names.greeting -r)
...
node_id=$(dml dag get examples/00-hello-world --value-type str | jq '.names.greeting' -r)
fndag_id=$(dml dag describe-node "${node_id}" | jq .dag -r)
cache_key=$(dml dag get "${fndag_id}" | jq .cache_key -r)
dml admin cache invalidate "${cache_key}" > /dev/null
