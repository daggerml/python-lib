...
TEST_FN_DIR = Path(__file__).resolve().parent / "_internal" / "fn"
...
class TestSetAttrs:
    ...
    def test_put_foreign_node_auto_imports(self, dml):
        with dml.new("d0", "d0") as dag0:
            dag0.commit(42)
        foreign_node = dml.load("d0").result
        with dml.new("d1", "d1") as dag1:
            imported_via_put = dag1.put(foreign_node, name="from_put")
            assert imported_via_put.value() == 42
            imported = dag1.load(foreign_node, name="from_d0")
            assert imported.value() == 42
            assert dag1["from_put"].value() == 42
            assert dag1["from_d0"].value() == 42
