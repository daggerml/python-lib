...
def test_value_context_type_equality_hash_and_repr(dag, fake_dml, refs):
...
def test_context_for_plain_node_returns_owning_dag(dag, fake_dml, refs):
...
def test_context_skips_builtin_and_root_recurses_through_nested_boundaries(dag, fake_dml):
...
def test_plain_node_is_not_callable(dag, refs):
...
def test_runnable_node_delegates_to_dag_call(dag, refs):
...
def test_collection_contains_and_len_delegate_to_builtin(dag, refs):
...
def test_list_index_slice_iter_conj_and_append(dag, refs):
...
def test_dict_index_keys_iter_get_items_values_assoc_update(dag, refs):
...
def test_dict_items_rejects_non_dict_type(dag, refs):
...
def test_committed_collection_reads_return_projection_and_materialize_paths(fake_dml, refs):
...
def test_projection_context_skips_builtin_collection_layers(dag, fake_dml):
    refs = SimpleNamespace(
...
def test_projection_rejects_callable_semantics(fake_dml, refs):
    committed = api.Dag(dml=fake_dml, ref=refs.dag, name="demo")
    projection = api.Projection(
        dag=committed,
