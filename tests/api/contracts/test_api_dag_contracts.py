...
def test_require_index_ref_rejects_committed_dag(fake_dml):
...
def test_make_node_classifies_value_shapes(fake_dml, refs):
...
def test_put_applies_codecs_and_wraps_ref(dag, fake_dml, refs):
...
def test_named_access_uses_partial_dag_description_for_uncommitted_dag(dag, fake_dml, refs):
    node = dag["a"]
...
def test_named_access_uses_description_for_committed_dag(fake_dml, refs):
...
def test_missing_committed_name_raises(fake_dml, refs):
    dag = api.Dag(dml=fake_dml, ref=refs.dag)
...
def test_set_named_node_with_raw_value_stages_value(dag, fake_dml, refs):
    dag.answer = 42

    fake_dml.runtime.put_literal.assert_called_with(refs.index, 42, name="answer")


def test_set_named_node_with_node_or_ref_updates_name(dag, fake_dml, refs):
...
def test_set_named_node_rejects_committed_dag(fake_dml, refs):
    dag = api.Dag(dml=fake_dml, ref=refs.dag)
...
def test_keys_values_len_and_iter_use_described_names(dag, refs):
    assert dag.keys() == ["a", "z"]
    assert len(dag) == 2
    assert list(dag) == ["a", "z"]
...
def test_argv_and_result_require_refs(dag, fake_dml, refs):
...
def test_result_raises_committed_terminal_error(fake_dml, refs):
...
def test_require_imports_result_and_named_nodes(dag, fake_dml, refs):
...
def test_require_reports_missing_dag_or_node(dag, fake_dml, refs):
...
def test_call_builtin_stages_runnable_and_non_ref_args(dag, fake_dml, refs):
...
def test_call_builtin_raises_when_execution_fails(dag, fake_dml):
...
def test_call_retries_until_result_and_times_out(dag, fake_dml, refs):
...
def test_commit_handles_raw_node_and_error_values(dag, fake_dml, refs):
    dag.commit(42)
...
def test_new_initializes_runtime_tags(fake_dml, refs):
    dag = api.new(dml=fake_dml, tags=["research.v0", "candidate", "candidate"])

    fake_dml.runtime.create.assert_called_once_with(
        cache_key=None,
        execution=None,
        tags=["research.v0", "candidate", "candidate"],
    )
    assert dag.token == refs.index


def test_tags_are_read_from_underlying_dag(dag, fake_dml, refs):
    fake_dml.dag.describe.return_value["tags"] = ["candidate", "research.v0"]

    assert dag.tags == ["candidate", "research.v0"]
    fake_dml.runtime.describe.assert_called_once_with(refs.index)
    fake_dml.dag.describe.assert_called_once_with(refs.dag)


def test_context_manager_commits_exceptions(dag):
...
def test_context_manager_rejects_committed_dag(fake_dml, refs):
    with pytest.raises(AssertionError):
        api.Dag(dml=fake_dml, ref=refs.dag).__enter__()


def test_private_missing_attribute_raises_attribute_error(dag):
    with pytest.raises(AttributeError, match="_missing"):
        _ = dag._missing


def test_repr_uses_ref_token_or_na(fake_dml, refs):
...
def test_freeze_and_unfreeze_replace_only_token_and_return_self(dag, fake_dml, refs):
...
def test_frozen_index_reads_use_partial_dag_projections(dag, fake_dml, refs):
    frozen = api.Ref("frozenindex:frozen")
    dag.token = frozen
...
def test_frozen_index_remains_uncommitted_and_does_not_auto_unfreeze(dag, fake_dml, refs):
    frozen = api.Ref("frozenindex:frozen")
    dag.token = frozen
...
def test_cancel_forwards_retry_budget_and_clears_token(dag, fake_dml, refs):
...
def test_cancel_failure_preserves_token(dag, fake_dml, refs):
...
def test_cancel_rejects_committed_dag(fake_dml, refs):
    dag = api.Dag(dml=fake_dml, ref=refs.dag)
