...
def test_accepted_selectors_parse_and_stringify(selector: str) -> None:
    ...
    assert revision.kind in {"head", "name", "commit", "project"}
...
@pytest.mark.parametrize("selector", ["", "HEAD~-1", "HEAD~x", "bad//name", "origin/main"])
