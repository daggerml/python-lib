...
- Current testing helper surface:
  - `MockNode`
  - `MockNode.from_value(...)`
  - `defunkify(...)`
- `MockNode` interface:
  - construction accepts exactly one wrapped Python value,
  - extra positional or keyword construction arguments are rejected,
  - `from_value(value) -> MockNode | Node` returns `value` unchanged when it is already a `Node` or `MockNode`, otherwise returns `MockNode(value)`,
  - extra positional or keyword arguments to `from_value(...)` are rejected,
  - `value() -> object` returns the wrapped value unchanged,
  - extra positional or keyword arguments to `value()` are rejected.
- `defunkify(delayed) -> callable`:
  - input MUST be a `DelayedRunnable`,
  - extra positional or keyword arguments to `defunkify(...)` are rejected,
  - supported input is limited to delayed-runnable wrapper chains whose innermost delayed runnable is script-backed and retains the original callable in `kwargs["fn"]`,
  - `defunkify(...)` MUST be the supported testing/debug path for recovering script callables from delayed runnable wrapper chains,
  - for supported input, `defunkify(...)` MUST traverse nested delayed-runnable wrappers to the innermost script-backed delayed runnable before constructing the returned callable,
  - the returned callable MUST execute inside an isolated temporary working directory rather than the caller's ambient current working directory,
  - returned callable MUST preserve the leading positional argument unchanged,
  - returned callable MUST bind defaults and MUST wrap every non-leading bound argument value that is not already a real `Node` or `MockNode` into `MockNode` before invoking the original callable,
  - returned callable MUST preserve `Node` and `MockNode` inputs rather than rewrapping them,
  - `defunkify(...)` MUST reject unsupported delayed-runnable inputs rather than guessing or synthesizing callable metadata,
  - delayed-runnable metadata unrelated to locating the supported innermost script callable is preserved on the input value and ignored by this helper,
  - the returned callable MUST preserve the wrapped callable's accepted argument names and defaulting behavior and MUST reject invocation patterns that the wrapped callable's signature does not accept.
- `MockNode` is a Node-Like Test Value only.
- `MockNode` MUST be usable in Author Code Unit Tests that rely only on `.value()` access.
- `MockNode` MUST NOT require repository state, database state, or active repository-backed runtime configuration.
- `MockNode` MUST NOT claim or emulate repository-backed `Node` identity, refs, persistence, DAG membership, or transactional behavior.
...
- Invalid `MockNode`/`MockNode.from_value(...)`/`MockNode.value()` invocation shape, including wrong argument count or unexpected keywords:
  - non-retryable until the call shape is corrected,
  - terminal and non-transient for that helper invocation,
  - caller behavior: call the helper with the documented positional-only shape for that interface,
  - operator action: fix the test call site; no runtime remediation exists.
- `defunkify(...)` with non-`DelayedRunnable` input:
  - non-retryable until the input value is corrected,
  - terminal and non-transient for that helper invocation,
  - caller behavior: pass a supported `DelayedRunnable` wrapper chain instead of a plain object,
  - operator action: fix the test/debug call site so it supplies a delayed-runnable wrapper chain.
- `defunkify(...)` with an innermost delayed runnable whose `uri` is not `"script"`:
  - non-retryable until the input value is corrected,
  - terminal and non-transient for that helper invocation,
  - caller behavior: pass a delayed-runnable wrapper chain whose innermost runnable is script-backed,
  - operator action: fix the helper input selection so `defunkify(...)` is used only for script-backed delayed-runnable chains.
- `defunkify(...)` with missing or non-callable innermost `kwargs["fn"]`:
  - non-retryable until the input value is corrected,
  - terminal and non-transient for that helper invocation,
  - caller behavior: pass a delayed-runnable wrapper chain that retains a callable innermost `kwargs["fn"]`,
  - operator action: fix the delayed-runnable construction or selection so callable metadata is retained.
- Invalid invocation of the callable returned by `defunkify(...)`, including unsupported argument binding for the wrapped callable signature:
  - non-retryable until the call shape is corrected,
  - terminal and non-transient for that invocation,
  - caller behavior: call the returned wrapper with arguments accepted by the wrapped callable's signature,
  - operator action: fix the test call site; no runtime remediation exists.
- Misuse of `MockNode` as a repository-backed `Node` substitute:
  - non-retryable within this helper surface,
  - terminal and non-transient for tests that require real DAG/runtime semantics,
  - caller behavior: use repository-backed APIs when tests require DAG membership, refs, or execution semantics.
  - operator action: switch the test to repository-backed APIs or reduce the test to contrib-facing `.value()` behavior only.
