...
class ExampleAdapter(AdapterBase):
    ...
    def resolve_runnable(cls, uri, kwargs, sub):
        return Runnable(target=Uri(uri), kwargs=kwargs, sub=sub, adapter=cls.name)
...
assert runnable.adapter == "example"
assert ExampleAdapter.send(operation="invoke") == {"status": "success", "error": None}
```
