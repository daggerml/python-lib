...
- Runnable model at execution boundary:
  ...
  - `kwargs` (`dict[str, Any]`),
  - `adapter` (string).
- Adapters are invoked via stdin JSON.
- Adapter stdin payload includes:
  - `argv_ptr`,
  - `cache_key`,
  - `remote` (`root`, `cache`),
  - `runnable`.
- Adapter stdin payload normalization is JSON-compatible and lossy:
  - `Uri` values become plain URI strings,
  - `Runnable` values become recursive JSON objects.
...
- Adapter output is JSON with schema:
