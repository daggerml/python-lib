...
<!-- docs:pseudocode: This incomplete method is intentionally shown only to illustrate the compiler's control-flow limitation; it requires its surrounding dagclass and worker state. -->
```python
...
See [Keep dependency syntax direct](start-here/dagclasses.qmd#keep-dependency-syntax-direct)
for executable examples and the complete reserved-name list.
...
<!-- docs:pseudocode: This external package implementation is a before-state comparison, not a standalone DaggerML example. -->
```python
# foo/bar.py
def mean(vals):
...
<!-- docs:pseudocode: This funk depends on the external foo package shown above and is included to explain cache identity rather than as an isolated runnable lesson. -->
```python
@api.funkify
...
and executes
<!-- docs:pseudocode: This assertion depends on the surrounding DAG and external package state used in the cache-key example. -->
```python
...
<!-- docs:pseudocode: This is the changed external dependency used to demonstrate why an existing cache key does not change. -->
```python
# foo/bar.py
def mean(vals):
    return sum(vals) / len(vals)
...
<!-- docs:pseudocode: This repeated funk is contextual to the external dependency-change scenario and has no isolated fixture dependency. -->
```python
@api.funkify
def compute_stats(dag, numbers):
    from foo.bar import variance
...
<!-- docs:pseudocode: These calls rely on the prior cache state and external foo package, which the isolated documentation fixture must not simulate. -->
```python
dag.call(compute_stats, [1, 2, 3]).value() == 2/3
...
<!-- docs:pseudocode: This source-inclusion pattern requires the external foo package and is presented as guidance for cache identity. -->
```python
...
<!-- docs:pseudocode: This optional NumPy variant depends on an external editable package and is not an isolated product workflow. -->
```python
# foo/bar.py
import numpy as np
...
<!-- docs:pseudocode: This intentionally malicious executable must never run in the documentation fixture; it demonstrates the adapter trust boundary. -->
```sh
#!/bin/sh
