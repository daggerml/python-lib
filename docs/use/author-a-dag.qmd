...
<!-- docs:pseudocode: This diagram illustrates an execution topology; it is not executable source. -->
```mermaid
flowchart TD
...
The forwarding of values like `preprocess_fn` is necessary to avoid the footgun specified [here](../sharp-bits-and-security.qmd#editable-dependency-changes-do-not-affect-a-funk-cache-key).
...
When `download_dataset` is staged, its `image` and `flags` configuration values resolve to the already-named nodes. The [Start here DAG lesson](../start-here/dags.qmd) builds the image node used by this pattern; add the model-training step appropriate to your workload.
...
<!-- docs:pseudocode: This is documented output, not executable source. -->
```text
...
The [Start here Dagclasses lesson](../start-here/dagclasses.qmd) provides a
runnable dagclass pipeline. Apply the same member-dependency rules when a
dagclass method has Docker configuration.
