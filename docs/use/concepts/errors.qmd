...
`DmlRepoError` covers invalid repository operations, configuration failures, and malformed state. Unlike a stored terminal error, it indicates that DaggerML could not complete the requested repository operation. See the [error reference](../reference/errors.qmd) for common messages and recovery actions.
