...
A funk is a DaggerML-packaged `Runnable`. Adding one to a DAG gives DaggerML the information it needs to run it. The current authoring tooling packages Python functions with `@api.funkify`, but a funk is not inherently tied to Python. Calling a funk from a DAG records DaggerML data for the computation and creates a result node. Its execution produces another DAG, so inputs, result, and failure can all be inspected later. Script funks may declare `tags=[...]`; those normalized tags are stored on every executed result DAG.
...
Normal completion cleanup is independent from result reuse. Cleanup is required while a result exists and its cleanup record is null. A caller returning either a fresh or cached terminal result gives eligible required cleanup one coordinated drive first; complete or failed cleanup is not repeated, and retry-delayed cleanup does not block or invalidate the DAG. Adapter `retry` responses store a shared `driver.not_before`, so all callers back off together. Cancellation is separate: it conditionally removes a selected execution's matching cache pointer before requesting cancellation teardown. `cancel-pending` means cancellation owns the attempt and blocks further result mutation; `canceled` means cancellation succeeded or no adapter work applied.
