...
`daggerml.Error` stores `message`, `origin`, `type`, and `stack`; it records a failed DAG terminal state. `DmlRepoError`, a subclass of `Error`, covers most project, configuration, runtime, and execution failures. See [Errors](errors.qmd) for error capture, function failures, inspection, and provenance.
