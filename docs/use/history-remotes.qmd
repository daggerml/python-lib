...
The documentation build supplies a disposable Moto S3 endpoint. This workflow
pushes the completed tutorial project, then fetches and checks out its `funks`
DAG in a separate local project:

```{bash}
dml config set remote.root "$DML_REMOTE_ROOT" >/dev/null
dml push >/dev/null

reuse_home="$DOCS_WORKSPACE_ROOT/remote-reuse"
mkdir "$reuse_home"
(
  cd "$reuse_home"
  dml init
  ...
  dml dag checkout main funks --remote --name fetched-funks
  dml show | jq -e '.dags["fetched-funks"]'
)
```

See [share and reuse research](share-reuse.qmd).
