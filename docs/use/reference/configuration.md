...
- `remote.remotes` (named project remotes, such as `origin -> dml://owner/project`)
...
Environment variables use the `DML_` prefix, including `DML_PROJECT_HOME`, `DML_REMOTE_ROOT`, `DML_REMOTE_PROJECT`, `DML_USER`, and `DML_CONFIG_HOME`. `remote.root` enables remote-backed execution and storage. `DML_REMOTE_PROJECT` configures the default `origin` remote for compatibility; use named remotes for project synchronization.
