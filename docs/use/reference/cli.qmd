...
Export one bundled, portable coding-agent skill with `dml skills querying > SKILL.qmd`, `dml skills authoring > SKILL.qmd`, `dml skills repository > SKILL.qmd`, or `dml skills extensions > SKILL.qmd`. Use `querying` for data extraction, DAG traversal, provenance, and persisted errors; use `repository` for cache inspection and invalidation.
