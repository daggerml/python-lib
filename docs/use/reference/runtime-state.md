...
`list` returns open runtimes in reverse creation order. `describe` reports the mutable graph and parent state. `describe-graph` reports reachable execution lineage; add `--visual` for a rendered view. Rendered terminal views require `pip install "daggerml[terminal]"`. `cancel` accepts `full` (the default) or `drive` mode.
