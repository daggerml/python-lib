...
<!-- docs:pseudocode: This conventional import is reference syntax, not an independently runnable lesson. -->
```python
...
<!-- docs:pseudocode: This projection pattern relies on nodes from a reader-authored DAG and is therefore reference syntax. -->
```python
...
<!-- docs:pseudocode: This low-level session pattern presumes an existing project and named dependency endpoint. -->
```python
...
<!-- docs:pseudocode: This cache and GC pattern requires reader-owned cache keys and a configured remote. -->
```python
...
For the higher-level `daggerml.contrib.api` helpers, including `funkify`, `ref`, `load`, `dagclass`, and `run`, see [author a DAG](../guides/author-a-dag.qmd).
