...
documentation from the [QMD source tree](../../start-here/index.qmd), without project or
revision scope. Docs navigation contains Start here, Concepts, Extend, and
Develop sections, with Glossary and Sharp bits and security as direct top-level
pages. It uses rendered page titles rather than filesystem labels and provides
a local page filter. Start here follows the same dependency-derived order used
to execute the course. Only the current section and Start here are expanded by
default so the complete inventory remains available without dominating the
page. Source blocks preserve syntax highlighting and expose an
