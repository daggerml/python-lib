---
title: Local Research Dashboard
engine: knitr
---
...
<!-- docs:pseudocode: command synopsis for the public dashboard launcher. -->
```text
...
equivalent light and dark themes. The DaggerML brand links to **Home**, and
**Docs** is the other global destination. Home combines the cross-project status
content and project directory; Docs serves packaged, version-matched product
documentation from the [QMD source tree](../../index.qmd), without project or
revision scope. Docs navigation groups pages by reader path, uses rendered page
titles rather than filesystem labels, and provides a local page filter. Within
each reader path, overview pages precede concepts, guides, and reference, with
subgroup labels preserving that hierarchy; first-DAG authoring belongs to the
introductory path. Only the current reader path and the introductory path are
expanded by default so the complete inventory remains available without
dominating the page. Source blocks preserve syntax highlighting and expose an
explicit top-right copy action. Executed output is visually labeled and
distinct from source. Mermaid diagrams render inside the packaged documentation
surface using the active dashboard theme; a rendering failure leaves the source
visible rather than hiding the diagram content.

Executable documentation runs in a fresh shared workspace for each build so
examples can build on earlier examples without reading or changing the user's
ambient project or configuration. `depends-on` page IDs define a validated,
acyclic execution graph. A page's workspace-relative `dml-project-home` must
already have been created by a dependency; the harness sets it as both
`DML_PROJECT_HOME` and the page working directory, without implicitly creating
or initializing a repository.

Selecting a locally available
project resolves its `HEAD` to
...
<!-- docs:pseudocode: canonical browser route grammar. -->
```text
/
/docs
/docs/:page
...
<!-- docs:pseudocode: collection response envelope. -->
```json
{
  "items": [],
  "next_cursor": null
...
<!-- docs:pseudocode: error response envelope. -->
```json
{
  "error": {
...
<!-- docs:pseudocode: cancellation confirmation request wire sample. -->
```http
...
- [Runtime state reference](../../use/reference/runtime-state.qmd)
