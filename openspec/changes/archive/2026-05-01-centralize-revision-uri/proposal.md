...
Revision URI parsing and canonicalization are currently duplicated across configuration and remote ops layers with near-identical rules and slightly different behavior boundaries. That duplication increases drift risk, makes policy changes expensive (like allowing `remote.project` tags), and creates unnecessary coupling between caller intent and URI representation details.
...
- Update project configuration semantics to allow tag-bearing `remote.project` values.
...
- Affected behavior: `remote.project` accepts branch or tag selectors; mutation restrictions remain enforced by branch/tag-specific operations.
