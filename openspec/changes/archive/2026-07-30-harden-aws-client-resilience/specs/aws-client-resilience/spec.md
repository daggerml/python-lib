...
The Batch executor SHALL use its high-resilience client policy for all Batch control-plane and scratch-S3 requests. That policy SHALL use a 60-second connection timeout, 60-second read timeout, adaptive retries with 10 attempts, and 100 maximum pooled connections.

#### Scenario: Batch job launch
- **WHEN** the Batch executor writes its input or registers and submits a Batch job
- **THEN** each AWS client used by those operations SHALL be configured with the Batch high-resilience policy
