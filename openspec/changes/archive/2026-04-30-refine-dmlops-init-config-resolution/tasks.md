...
- [x] 1.3 Ensure required init-time values fail fast with explicit errors when unresolved/invalid (including `project.uri` and `remote.uri` when required by bootstrap flow).
...
- [x] 2.2 Trigger pull during recovery when resolved config includes `project.uri`, using resolved remote/project context.
- [x] 2.3 Ensure recovery path skips pull when no `project.uri` is configured and still completes local init successfully.
