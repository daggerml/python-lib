...
- **WHEN** `DmlOps.init` recovers a missing DB and resolved config includes `remote.project`
...
- **WHEN** `DmlOps.init` recovers a missing DB and resolved config has no `remote.project`
