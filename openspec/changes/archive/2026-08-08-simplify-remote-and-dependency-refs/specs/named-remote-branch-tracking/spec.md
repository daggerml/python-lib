...
**Reason**: Project synchronization and execution use the sole configured `remote.root`; only import dependencies are named.
**Migration**: Configure the project endpoint through `remote.root` and register import-only sources with `dml dep add`.
