...
- **WHEN** init is called with both `name` and `project_uri`
...
The init operation MUST allow `name` to be omitted when `project_uri` is provided and MUST initialize project identity from the explicit URI.

#### Scenario: Project URI without name
- **WHEN** init is called with `project_uri` and no `name`
...
When init is called with `name` and without `project_uri`, the system MUST resolve the global config user and derive the canonical project URI from that user and the provided name.

#### Scenario: Name-only init with resolved user
- **WHEN** init is called with `name`, no `project_uri`, and a resolvable global config user
...
When init is called with `name` and without `project_uri`, and global config user cannot be resolved, init MUST fail with a descriptive configuration error explaining that name-based init requires a resolved user identity.

#### Scenario: Name-only init with unresolved user
- **WHEN** init is called with `name`, no `project_uri`, and no resolvable global config user
