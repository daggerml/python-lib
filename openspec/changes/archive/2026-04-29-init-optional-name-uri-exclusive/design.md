...
- **Single identity source gate in `DmlOps.init`**: Centralize validation so exactly one of (`name`, `project_uri`) is used for project identity. This avoids duplicated logic between CLI and internal call paths.
...
- **Preserve explicit URI authority**: If `project_uri` is provided, treat it as authoritative and do not derive from `name`.
...
- **[Trade-off] Stricter validation may surface misconfigurations earlier** → Mitigation: provide actionable error text including required mode (`--name` vs `--project-uri`) and missing user guidance.
...
- Add or update tests for:
  ...
  - Rejection of simultaneous `name` and `project_uri`.
