...
The generated CLI SHALL expose `dml skills authoring`, `dml skills repository`, and `dml skills inspection`. Each command SHALL write its complete corresponding bundled skill document to standard output and SHALL not write command framing or serialized representation around that document. The CLI SHALL NOT expose `dml admin agent-skill` or an `admin` namespace.
...
- **WHEN** a user runs each of `dml skills authoring`, `dml skills repository`, and `dml skills inspection`
