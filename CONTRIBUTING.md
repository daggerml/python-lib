...
the documentation build, and deploys the standalone site to GitHub Pages.
Pull requests, tags, other branches, and unsuccessful CI runs do not deploy.
In the repository's **Settings → Pages**, set the build source to **GitHub Actions**.
The default site URL is <https://daggerml.github.io/python-lib/>.
...
`--no-docs`. The output contains complete HTML pages, Quarto assets, a root entry
page, and relative navigation links that work beneath a repository URL prefix.
Serve that directory with a static HTTP server to preview it.

The standalone site also includes a searchable **API reference** at `api/`,
...
code. Tutorial pages link to the reference, and API pages link back to the
tutorials. pdoc is a development dependency; API generation runs only when
`DOCS_SITE_OUTPUT` is set.
